...
export default {
    ...
    async execute(ctx) {
        ...
        if (mentions.length === 0 || args.length < 1) {
            return await ctx.reply('✘ Debes mencionar a un usuario.\n\nEjemplo:\n*#dar @usuario id123*');
        }
        ...
        if (!characterId) {
            return await ctx.reply('✘ Debes proporcionar el ID del personaje.\n\nEjemplo:\n*#dar @usuario id123*');
        }
        ...
        try {
            ...
            if (!character) {
                return await ctx.reply(`✘ No se encontró ningún personaje con el ID: *${characterId}*`);
            }
            ...
            await ctx.reply(`✘ Error al dar el personaje: ${error.message}`);
        }
    }
};
