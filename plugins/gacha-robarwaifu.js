export default {
    ...
    async execute(ctx) {
        ...
        if (!args[0]) {
            return await ctx.reply('✘ Debes proporcionar el ID de la waifu que quieres robar.\n\nEjemplo:\n*#robarwaifu id123*');
        }
        ...
        try {
            ...
            if (!waifu) {
                return await ctx.reply(`✘ No se encontró ninguna waifu con el ID: *${waifuId}*`);
            }

            const oldOwner = waifu.user;

            if (!oldOwner || oldOwner === ctx.sender) {
                return await ctx.reply('✘ Esta waifu no tiene dueño o ya es tuya.');
            }

            if (oldOwner === global.botOwner) {
                return await ctx.reply(`✘ No puedes robar la waifu de mi owner *${waifu.name}* (ID: ${waifu.id}).`);
            }
            ...
            if (oldOwner && oldOwner !== ctx.sender && oldOwner !== global.botOwner) {
                try {
                    await ctx.bot.sock.sendMessage(oldOwner, {
                        text: `✘ El usuario *@${ctx.sender.split('@')[0]}* ha robado tu waifu *${waifu.name}* (ID: ${waifu.id}).`,
                        mentions: [ctx.sender]
                    });
                } catch (error) {
